...
public class SetStatementParserListener extends MySqlParserBaseListener {
    ...
    public void enterOptionValueNoOptionType(MySqlParser.OptionValueNoOptionTypeContext ctx) {
        ...
        else if (ctx.charsetClause() != null) {
            String charsetName = ctx.charsetClause().charsetName() != null
                    ? parser.withoutQuotes(ctx.charsetClause().charsetName().getText())
                    : parser.currentDatabaseCharset();
